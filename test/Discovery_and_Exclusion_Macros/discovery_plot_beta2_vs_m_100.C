...
void makePlots()
{
 ...
 string fileName = "beta2_vs_m.eps";
 ...
 for(Int_t i = 0; i < size; i++){
  ...
  beta2[i] = beta2_for_5sigma(NPts, rangeMin[i], rangeMax[i], N_sig_100[i], N_bkg_100[i], 0, mass[i]);
  if(systematics) beta2_sys[i] = beta2_for_5sigma(NPts, rangeMin_sys[i], rangeMax_sys[i], N_sig_100[i], N_bkg_100[i], Sigma_N_bkg[i], mass[i]);
 }

 TGraph *beta2_vs_m = new TGraph(size, m, beta2);
 TGraph *beta2_vs_m_fit = new TGraph(size, m, beta2);
 ...
 TGraph *beta2_vs_m_sys_fit = new TGraph(size, m, beta2_sys);
 ...
 if(systematics) beta2_vs_m_sys_fit->Fit("fit2");
 if(systematics) {fit2->SetRange(200,600); fit2->Draw("same");}

 TLegend *legend = new TLegend(.27,.72,.72,.85);
 legend->SetBorderSize(1);
 legend->SetFillColor(0);
 //legend->SetFillStyle(0);
 legend->SetMargin(0.2);
 legend->SetHeader("LQ #rightarrow eq");
 legend->AddEntry(beta2_vs_m_sys,"5#sigma_{S_{cP}} (with sys. unc.)","lp");
 legend->AddEntry(beta2_vs_m,"5#sigma_{S_{cP}} (no sys. unc.)","lp");
