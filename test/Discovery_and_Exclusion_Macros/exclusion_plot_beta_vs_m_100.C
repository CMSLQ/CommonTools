...
//  Double_t beta_sys[size]; // with systematics included
 Double_t beta_sys[5] = {0.177045, 0.234562, 0.406596, 0.683961, 1.15621}; // with systematics included
 
//  for(Int_t i = 0; i < size; i++){
//   
//   beta[i] = beta_for_exclusion(NPts, xsTh[i], rangeMin[i], rangeMax[i], 0, S_eff[i], 0, N_bkg_100[i], 0, mass[i]);
//   if(systematics) beta_sys[i] = beta_for_exclusion(NPts, xsTh[i], rangeMin_sys[i], rangeMax_sys[i], Sigma_L, S_eff[i], Sigma_S_eff[i], N_bkg_100[i], Sigma_N_bkg[i], mass[i]);
//  }
 ...
 TLegend *legend = new TLegend(.24,.68,.67,.85);
 legend->SetBorderSize(1);
 legend->SetFillColor(0);
 //legend->SetFillStyle(0);
 legend->SetMargin(0.2);
 legend->SetHeader("LQ #rightarrow eq");
 legend->AddEntry(beta_vs_m_sys,"95% C.L. (with sys. unc.)","lp");
 legend->AddEntry(beta_vs_m,"95% C.L. (no sys. unc.)","lp");
